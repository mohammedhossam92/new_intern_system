/*
  # Initial Database Schema for Dental System

  1. New Tables
    - `users` - User profiles extending Supabase auth.users
      - Links to auth.users with CASCADE delete
      - Stores role (Intern/Student, Doctor, Admin)
      - Profile information (name, phone, university, etc.)
    
    - `internship_periods` - Tracks student internship periods
      - Links to users (student and supervisor)
      - Tracks hours completed and required hours
      - Status tracking (pending, in_progress, completed, approved)
    
    - `patients` - Patient records
      - Basic information (name, contact, DOB)
      - Medical history and allergies
      - Status for approval workflow (pending, approved, rejected)
    
    - `treatments` - Treatment records
      - Links to patient, student, and supervisor
      - Treatment details with tooth numbers array
  ...
  2. Security
    ...
    - Users can view all profiles, update own profile or admin can update any
    - Internship periods accessible to student, supervisor, doctors, and admins
    - Patients accessible to creator, doctors, admins, and assigned students
    - Treatments accessible to student, supervisor, doctors, and admins
    - Notifications only accessible to recipient user
    - Appointments accessible to student, supervisor, doctors, and admins

  3. Indexes
    - Performance indexes on foreign keys and commonly queried columns

  4. Triggers
    - Auto-notify on treatment status changes
    - Auto-notify on new patient additions
    - Auto-notify on appointment creation

  5. Functions
    - Helper functions for querying student/supervisor relationships
*/
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  related_entity_type VARCHAR(20) CHECK (related_entity_type IN ('patient', 'treatment', 'user', 'internship', 'appointment')),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_internship_periods_user_id ON internship_periods(user_id);
CREATE INDEX IF NOT EXISTS idx_internship_periods_supervisor_id ON internship_periods(supervisor_id);
...
CREATE INDEX IF NOT EXISTS idx_treatments_student_id ON treatments(student_id);
CREATE INDEX IF NOT EXISTS idx_treatments_supervisor_id ON treatments(supervisor_id);
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'internship_periods' AND policyname = 'internship_periods_select_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'internship_periods' AND policyname = 'internship_periods_insert_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'internship_periods' AND policyname = 'internship_periods_update_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'patients' AND policyname = 'patients_select_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'patients' AND policyname = 'patients_insert_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'patients' AND policyname = 'patients_update_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'appointments' AND policyname = 'appointments_select_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'appointments' AND policyname = 'appointments_insert_policy') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'appointments' AND policyname = 'appointments_update_policy') THEN
...
DROP TRIGGER IF EXISTS treatment_status_change_trigger ON treatments;
...
DROP TRIGGER IF EXISTS patient_added_trigger ON patients;
